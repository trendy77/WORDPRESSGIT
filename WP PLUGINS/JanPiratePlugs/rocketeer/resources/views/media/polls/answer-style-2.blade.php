@foreach($qv->answers as $ak => $av)
    @if(isset($qv->already_answered))
        <div class="poll-answer-ctr poll-answer-style-2 @if($qv->answer_key == $ak) poll-answer-selected @endif">
    ...
    @else
        <div class="poll-answer-ctr poll-answer-style-2" data-total-votes="{{ $av->total_votes }}">
            @if(!empty($av->img))<img src="{{ jasko_component( '/uploads/' . $av->img ) }}">@endif
            {{ $av->answer }}
        </div>
    @endif
