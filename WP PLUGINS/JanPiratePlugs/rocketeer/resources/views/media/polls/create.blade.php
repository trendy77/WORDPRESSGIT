            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-body">
                        <h2>{{ trans('media.create_poll') }}</h2>
                        <div id="alertStatus"></div>
                        <form id="createPollForm" novalidate="novalidate">
                            {!! csrf_field() !!}
                            <div class="form-steps-ctr">
                                <div class="form-steps-group">
                                    ...
                                    <div class="form-step"><h4><i class="fa fa-puzzle-piece"></i> 2</h4><span>{{ trans('media.poll_details') }}</span></div>
                                </div>
                                <div class="single-step-ctr" id="basic-details-step">
                                    ...
                                    <div class="form-group"
                                        @if($settings->media_overrides->poll_page_content == 2 || $user->isAdmin == 2 || $user->isMod == 2) style="display:block;" @else style="display:none;" @endif>
                                ...
                                <div class="single-step-ctr" id="poll-details-step" style="display: none;">
                                    <div class="form-group"
                                         @if($settings->media_overrides->poll_style == 2 || $user->isAdmin == 2 || $user->isMod == 2) style="display:block;" @else style="display:none;" @endif>
                                        <label for="inputStyle">{{ trans('media.style') }}</label>
                                        <select class="form-control" id="inputStyle">
                                            <option value="1">{{ trans('media.style') }} 1</option>
                                            <option value="2" {{ $settings->media_defaults->poll_style == 2 ? 'selected': '' }}>{{ trans('media.style') }} 2</option>
                                        </select>
                                    </div>
                                    <div class="form-group"
                                         @if($settings->media_overrides->poll_animation == 2 || $user->isAdmin == 2 || $user->isMod == 2) style="display:block;" @else style="display:none;" @endif>
                                        ...
                                        <select class="form-control" id="inputAnimation">
                                            ...
                                            @foreach($animations as $a)
                                                <option value="{{ $a }}" {{ $settings->media_defaults->poll_animation == $a ? 'selected': '' }}>{{ $a }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="inputThumbnail">{{ trans('media.thumbnail') }}</label>
                                        <select class="form-control" id="inputThumbnail">
                                            <option value="0">----- {{ trans('media.select_thumbnail') }} -----</option>
                                    ...
                                    <div class="row">
                                        <div class="col-md-5">
                                            ...
                                            <div id="pollDropzone">
                                                <i class="fa fa-cube fa-3x"></i><br><em>({{ trans('media.drag_drop_images') }})</em>
                                            </div>
                                            <div id="pollImgPreviewCtr"></div>
                                        </div>
                                        <div class="col-md-7">
                                            <h3>
                                                {{ trans('media.questions') }}
                                                <button type="button" class="btn btn-purple pull-right" v-on:click="addQuestion"><i class="fa fa-plus"></i> {{ trans('general.add') }}</button>
                                            ...
                                            <div>
                                                <div v-for="(index, question) in questions" class="mediaEditItemOverviewCtr">
                                                    ...
                                                    <button type="button" class="btn btn-xs btn-danger pull-right"
                                                            v-on:click="removeQuestion(index, $event)"><i class="fa fa-remove"></i></button>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title"><i class="fa fa-pencil"></i> {{ trans('media.edit_question') }}</h4>
                                        </div>
                                        <div class="modal-body">
                                            ...
                                            <h3>
                                                {{ trans('media.answers') }}
                                                <button type="button" class="btn btn-purple pull-right btn-sm" v-on:click="addAnswer">
                                            ...
                                            <div v-for="(index, answer) in questions[question_index].answers" class="answerCtr form-horizontal">
                                                <div class="form-group">
                                                    ...
                                                    <div class="col-sm-8"><input type="text" class="form-control" placeholder="Answer" v-model="answer.answer"></div>
                                                    <div class="col-sm-2">
                                                        <button type="button" class="btn btn-sm btn-danger" v-on:click="removeAnswer(index, $event)"><i class="fa fa-remove"></i></button>
                                                    </div>
                                                </div>
                                                <div class="form-group">
...
@section( 'scripts' )
    ...
    <script src="{{ jasko_component('/components/core/js/create-poll.min.js') }}"></script>
@endsection