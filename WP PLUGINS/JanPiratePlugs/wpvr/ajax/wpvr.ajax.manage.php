	...
	function wpvr_refresh_manage_videos_ajax_function() {
		...
		foreach ( (array) $return['items'] as $item ) {
			...
			if ( $dups ) {
				...
				$item_thumb_src = wp_get_attachment_image_src( get_post_thumbnail_id( $item->post_id ), 'wpvr_hard_thumb' );
				$item_thumb_img = '<img class="wpvr_video_thumb_img" src="' . $item_thumb_src[0] . '" />';
				//$item_thumb_img = '';
				//_d( get_post_thumbnail_id( $item->post_id ) );
			}
