<?php
	...
	function __wpvr_youtube_fetch_thumbnails( $video_id ) {
		global $wpvr_vs;
		$vs = $wpvr_vs['youtube'];
		...
		return wpvr_youtube_get_thumbnails( $api_response['json']['items'][0] );
		
	}
	
	
	function __wpvr_youtube_fetch_item_thumbnail( $item ) {
		global $wpvr_vs;
		$vs       = $wpvr_vs['youtube'];
		$video_id = is_string( $item->id ) ? $item->id : $item->id->videoId;
		$thumbs   = array(
			'icon'    => WPVR_NO_THUMB,
			'thumb'   => WPVR_NO_THUMB,
			'hqthumb' => WPVR_NO_THUMB,
	...
	function __wpvr_youtube_get_thumbnails( $item ) {
		...
		//d( $item->snippet->thumbnails );
		if ( isset( $item->snippet->thumbnails->default ) ) {
			$thumbs['icon'] = $item->snippet->thumbnails->default->url;
		}
		
		if ( isset( $item->snippet->thumbnails->medium ) ) {
			$thumbs['thumb'] = $item->snippet->thumbnails->medium->url;
		}
		
		if ( isset( $item->snippet->thumbnails->maxres ) ) {
			$thumbs['hqthumb'] = $item->snippet->thumbnails->maxres->url;
		} elseif ( isset( $item->snippet->thumbnails->high ) ) {
			$thumbs['hqthumb'] = $item->snippet->thumbnails->high->url;
		} else {
			$thumbs['hqthumb'] = $thumbs['thumb'];
		}
	...
	$vs['get_single_video_data'] = function ( $video_id ) use ( $vs, $pid_suffix ) {
		...
		$metas['likes']            = $item->statistics->likeCount;
		$metas['dislikes']         = $item->statistics->dislikeCount;
