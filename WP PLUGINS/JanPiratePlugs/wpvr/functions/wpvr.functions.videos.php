<?php
	...
	if ( ! function_exists( 'wpvr_render_modified_player' ) ) {
		function wpvr_render_modified_player( $post_id = null, $second_call = false ) {
			...
			if ( is_single() ) {
				$auto_play = $wpvr_options['playerAutoPlay'];
			} else {
				$auto_play = false;
			}
			
			$wpvr_player_tags = $wpvr_dynamics['player_tags'];
			$wpvr_player_tags = apply_filters( 'wpvr_extend_player_tags', $wpvr_player_tags );
			
			$wpvr_video_id = get_post_meta( $post->ID, 'wpvr_video_id', true );
			$wpvr_service  = get_post_meta( $post->ID, 'wpvr_video_service', true );
			...
			$wpvr_player = wpvr_video_embed(
				$video_id = $wpvr_video_id,
				$post_id = $post->ID,
				$auto_play,
				$service = $wpvr_service,
				$add_style = false,
			...
			//d( $wpvr_player );
			/*d( $video_id );d( $wpvr_service );*/
			
			$embedCode = '';
			$embedCode .= $wpvr_player_tags['before_outer'];
			$embedCode .= '<div class="wpvr_embed wpvr_new ' . $wpvr_player_tags['embed_class'] . '">';
			$embedCode .= $wpvr_player_tags['before_inner'];
			$embedCode .= $wpvr_player;
			$embedCode .= $wpvr_player_tags['after_inner'];
			$embedCode .= '</div>';
			$embedCode .= $wpvr_player_tags['after_outer'];
			
			if ( $second_call === false ) {
				return apply_filters( 'wpvr_extend_video_player_rendering', $embedCode, $post_id );
			} else {
				return $embedCode;
			}
		}
	...
	if ( ! function_exists( 'wpvr_add_video' ) ) {
		function wpvr_add_video( $videoItem, $_wpvr_imported = array(), $allowDuplicates = false ) {
			...
			if ( isset( $wpvr_imported[ $videoItem['service'] ][ $videoItem['id'] ] ) ) {
				return false;
			}
			
			/* If this video is already imported => Don't do anything */
			if ( $allowDuplicates === false ) {
				if ( isset( $_wpvr_imported[ $videoItem['service'] ][ $videoItem['id'] ] ) ) {
					return false;
				}
			}
			//d( $videoItem );
			...
			wpvr_add_log( array(
				...
				//Y-m-d H:i:s
				"action"   => __( 'ADDING A VIDEO', WPVR_LANG ) . ' ( ' . $videoItem['origin'] . ' )',
				...
				//
				"log_msgs" => array(
					'<b>' . __( 'SERVICE', WPVR_LANG ) . ' </b>: ' . strtoupper( $videoItem['service'] ),
					'<b>' . __( 'TITLE', WPVR_LANG ) . ' </b>: ' . $videoItem['title'],
					'<b>' . __( 'POSTED AS', WPVR_LANG ) . ' </b>: ' . $video_log_status,
					'<b>' . __( 'NEW POST ID', WPVR_LANG ) . ' </b>: ' . $newPostId,
				),
			) );
	...
	function wpvr_get_video_information( $post_id ) {
		...
		$video_info              = array(
			...
			'thumb_img'        => get_the_post_thumbnail( $post_id, 'wpvr_hard_thumb' ),
			'comment_count'    => $comments->total_comments,
		);
		$video_info['views']     = $video_info['views'] == '' ? 0 : $video_info['views'];
		$video_info['thumb_img'] = $video_info['thumb_img'] == '' ? '<img src="' . WPVR_NO_THUMB . '" />' : $video_info['thumb_img'];
