	...
	if ( ! class_exists( 'wpvr_videos_bulk_actions' ) ) {
		class wpvr_videos_bulk_actions {
			...
			function bulk_create_menus() {
                ...
                <script type="text/javascript">
                    jQuery(document).ready(function () {
                        ...
                        jQuery('<option>').val('update_thumbs').text('- <?php _e( 'Update Thumbnails', WPVR_LANG )?>').appendTo("select[name='action']");
                        jQuery('<option>').val('update_thumbs').text('- <?php _e( 'Update Thumbnails', WPVR_LANG )?>').appendTo("select[name='action2']");
			...
			function bulk_delete_permanently( $ids ) {
				...
				wpvr_render_done_notice_redirect( '<strong>' . $k . '</strong> ' . __( 'videos deleted successfully.', WPVR_LANG ), true );
