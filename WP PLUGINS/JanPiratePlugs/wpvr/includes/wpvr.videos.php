<?php
			...
			$save_button
				...
				<div class="">
					...
					<button
						...
						video_id = "' . $video_id . '"
						service = "' . $video_service . '"
						class="wpvr_black_button wpvr_full_width wpvr_button wpvr_video_update"
					>
						<i class="fa fa-save" style="margin-right:5px;"></i>
						' . __( 'Update video', WPVR_LANG ) . '
					</button>
				</div>
			';
	...
	function wpvr_video_filter_by_author( $query ) {
		...
		if (
			...
			&& is_admin()
			&& $pagenow == 'edit.php'
			&& isset( $_GET['video_service'] )
			&& $_GET['video_service'] != ''
			&& $_GET['video_service'] != '0'
		) {
			$meta_query[] = array(
				'key'   => 'wpvr_video_service',
				'value' => $_GET['video_service'],
			);
		}
	...
	function wpvr_load_video_hooks() {
		...
		if ( apply_filters( 'wpvr_extend_define_videos_columns', true ) ) {
			...
			function wpvr_video_custom_columns( $column ) {
				...
				//
				// if ( $video_info['service'] == '' || ! isset( $wpvr_vs[ $video_info['service'] ] ) ) {
				//
				//     return false;
				// }
				switch ( $column ) {
					...
					case 'video_meta':
						...
						$echo .= __( 'Posted', WPVR_LANG ) . ' <b>' . $video_info['post_date'] . '</b> <br/>';
						$echo .= '</span>';
